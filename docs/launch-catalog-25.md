...
| 17 | Fin-R1 | Financial reasoning | Apache-2.0 declared by the upstream model card | Pinned trusted community Q4_K_M converted from the official weights | Pure-Java accepted through a mandatory exact-artifact Qwen2/ChatML test. The marker retains the financial warning and card-only license provenance. |
| 18 | EuroLLM 1.7B Instruct | Multilingual/translation | Apache-2.0 | Pinned community Q4_K_M with structured official-checkpoint lineage | Pure-Java accepted through a mandatory exact-artifact Llama/SentencePiece/ChatML test. The marker retains the upstream alignment warning. |
