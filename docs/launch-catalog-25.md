...
The catalog currently contains 18 GGUF marker artifacts representing 16
...
| 16 | HuatuoGPT-o1-7B | Medical reasoning | Apache-2.0 | Pinned trusted community Q4_K_M converted from the official weights | Marker and [acceptance evidence](model-evidence/huatuogpt-o1-7b-q4-k-m.md) added with `pure-java=false`. Qwen2.5-7B architecture and tokenizer reuse the current backend; add the real fixture and no-clinical-use runtime contract before enabling pure Java. |
| 17 | Fin-R1 | Financial reasoning | Apache-2.0 declared by the upstream model card | Pinned trusted community Q4_K_M converted from the official weights | Marker and [acceptance evidence](model-evidence/fin-r1-7b-q4-k-m.md) added with `pure-java=false`. The artifact, ChatML oracle, financial warning, and card-only license provenance are verified; add the mandatory real fixture before enabling pure Java. |
| 18 | EuroLLM 1.7B Instruct | Multilingual/translation | Apache-2.0 | Pinned community Q4_K_M with structured official-checkpoint lineage | Marker and [acceptance evidence](model-evidence/eurollm-1-7b-instruct-q4-k-m.md) added with `pure-java=false`. The artifact, 35-language metadata, SentencePiece tokenizer, ChatML translation oracle, and upstream alignment warning are verified; add the mandatory real fixture before enabling pure Java. |
...
| Multilingual | [EuroLLM 1.7B Instruct](https://huggingface.co/utter-project/EuroLLM-1.7B-Instruct) and [verified GGUF](https://huggingface.co/mradermacher/EuroLLM-1.7B-Instruct-GGUF) | Apache-2.0 model covering 35 languages, with a paper, 101 likes, about 11.6K monthly upstream downloads, and 13 quantizations | Launch marker accepted with immutable Q4_K_M lineage and `pure-java=false`. Preserve the warning that the model is not preference-aligned and may generate harmful, hallucinated, or false content. |
| Finance | [Fin-R1](https://huggingface.co/SUFE-AIFLM-Lab/Fin-R1) and [trusted GGUF](https://huggingface.co/bartowski/SUFE-AIFLM-Lab_Fin-R1-GGUF) | Qwen2 financial-reasoning release with a paper, 229 upstream likes, 733 upstream monthly downloads, and a trusted 4.68 GB Q4_K_M conversion with 2.4K monthly downloads | Launch marker accepted with immutable lineage and `pure-java=false`. The model card declares Apache-2.0 but lacks structured license metadata and a LICENSE file; preserve that provenance warning and require ChatML because raw completion failed the arithmetic smoke test. |
...
- [Fin-R1](https://huggingface.co/SUFE-AIFLM-Lab/Fin-R1)
- [Fin-R1 trusted GGUF](https://huggingface.co/bartowski/SUFE-AIFLM-Lab_Fin-R1-GGUF)
- [EuroLLM 1.7B Instruct](https://huggingface.co/utter-project/EuroLLM-1.7B-Instruct)
- [EuroLLM 1.7B Instruct verified GGUF](https://huggingface.co/mradermacher/EuroLLM-1.7B-Instruct-GGUF)
