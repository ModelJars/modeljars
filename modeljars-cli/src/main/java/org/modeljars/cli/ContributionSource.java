  ...
  static ContributionSource parse(String input) {
    ...
    if (value.startsWith("hf://")) {
      ...
      throw new IllegalArgumentException(
          "Only Hugging Face repository sources are currently supported");
    }
    while (value.endsWith("/")) value = value.substring(0, value.length() - 1);
    String[] parts = value.split("/", -1);
    if (parts.length != 2
        || !COMPONENT.matcher(parts[0]).matches()
        || !COMPONENT.matcher(parts[1]).matches()) {
      throw new IllegalArgumentException(
          "Expected a Hugging Face repository in OWNER/REPOSITORY form: " + input);
    }
